                                            ...
                                            size_t table_len, void *data)
{
    ...
    for (i = 0; i < table_len; i++) {
        ...
        storage = (void *)((char *)data + table[i].storage_offset);
        ret = table[i].parser_func(c->value, c->value_len, storage, table[i].storage_len);
        if (ret < 0) {
...
int ConfFile::loop_sub_sections(const char *section_name, char separator, const char **sub_name,
                                size_t *sub_len, const OptionsTable table[], size_t table_len,
                                void *data, void **ptr)
{
    struct section *s;
    ...
    for (; s; s = s->next) {
        if (s->len - 1 > section_name_len && strncaseeq(section_name, s->name, section_name_len)
            && s->name[section_name_len] == separator) {
            *sub_name = s->name + section_name_len + 1;
            *sub_len = s->len - (*sub_name - s->name);
            *ptr = s;
            return _extract_options_from_section(s, table, table_len, data);
        }
    }
