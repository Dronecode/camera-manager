...
int UDPSocket::do_write(const struct buffer &buf)
{
    if (fd < 0) {
    ...
    ssize_t r = ::sendto(fd, buf.data, buf.len, 0, (struct sockaddr *)&sockaddr, sizeof(sockaddr));
    if (r == -1) {
...
int UDPSocket::do_read(const struct buffer &buf)
{
    socklen_t addrlen = sizeof(sockaddr);
    ssize_t r = ::recvfrom(fd, buf.data, buf.len, 0, (struct sockaddr *)&sockaddr, &addrlen);
    if (r == -1) {
        log_debug("error");
        if (errno != EAGAIN)
